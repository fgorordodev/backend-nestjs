...
import { REQUEST_ID_HEADER, UUID_PATTERN } from './request-context.constants';
...
export class RequestContextMiddleware implements NestMiddleware {
  ...
  use(request: Request, response: Response, next: NextFunction): void {
    const assignedRequestId = response.getHeader(REQUEST_ID_HEADER);

    const incomingRequestId = request.get(REQUEST_ID_HEADER);

    const requestId = this.isAssignedRequestId(assignedRequestId)
      ? assignedRequestId
