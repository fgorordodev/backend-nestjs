...
const SILENT_SUCCESS_PATHS = new Set(['/health/live', '/health/ready']);
...
export class RequestLoggingMiddleware implements NestMiddleware {
  ...
  use(request: Request, response: Response, next: NextFunction): void {
    ...
    const logRequest = (aborted: boolean): void => {
      ...
      const durationMs = this.calculateDuration(startedAt);

      const event = {
        event: aborted ? 'http_request_aborted' : 'http_request_completed',
  ...
  private calculateDuration(startedAt: bigint): number {
    ...
    return Number(elapsedMilliseconds.toFixed(2));
  }
}
